...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        return playCard_Mine(state, handPos, currentPlayer, choice1, choice2);
    ...
    case baron:
        return playCard_Baron(state, currentPlayer);
    ...
    case minion:
        return playCard_Minion(state, choice1, currentPlayer);
    ...
    case tribute:
        return playCard_Tribute(state, nextPlayer, currentPlayer);

    case ambassador:
        return playCard_Ambassador(state, choice1, choice2, handPos, currentPlayer);
...
int playCard_Baron(struct gameState* state, int currentPlayer)
{
...
int playCard_Minion(struct gameState* state, int choice1, int currentPlayer)
{
    //+1 action
    state->numActions++;
...
int playCard_Ambassador(struct gameState* state, int choice1, int choice2, int handPos, int currentPlayer)
{
    j = 15;		//used to check if player has enough cards to discard
...
int playCard_Tribute(struct gameState* state, int nextPlayer, int currentPlayer)
{
    ...
    if (tributeRevealedCards[0] == tributeRevfards[1]) { //If we have a duplicate card, just drop one
        state->playedCards[state->playedCardCf= tributeRevealedCards[1];
        state->playedCardCount++;
        tributeRevealedCards[1] = tributeRevealedCards[0];
    }
...
int playCard_Mine(struct gameState* state, int handPos, int currentPlayer, int choice1, int choice2)
{
    j = state->hand[currentPlayer][choice1];  //store card we will trash
